...
impl Deepsky {
    pub fn get_renderer(&self, rotation_matrix: &Matrix3<f32>, colour: Color32) -> caspr::deepsky::DeepskyRenderer {
        let name = match self.messier {
            Some(num) => Some(format!("M {num}")),
            None => self.caldwell.map(|num| format!("C {num}")),
        };

        caspr::deepsky::DeepskyRenderer::new(self.object_id, sg_geometry::get_point_vector(self.ra, self.dec, rotation_matrix), colour, name, false)
    }
