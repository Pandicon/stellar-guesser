            ...
            if let Ok(response_res) = check_updates_setup.receiver.try_recv() {
                match response_res {
                            ...
                            CheckUpdatesShowPopup::OnFoundUpdate => {
                                if response.newer_version_exists {
                                    self.toasts
                                        ...
                                        .duration(Some(web_time::Duration::from_secs(15)));
                                }
                            }
                            CheckUpdatesShowPopup::IfInfoWindowClosed => {
                                if !self.state.windows.app_info.opened {
                                    if response.newer_version_exists {
                                        ...
                                        self.toasts.info("No updates found.").duration(Some(web_time::Duration::from_secs(15)));
                                    }
                                }
                            }
                        }
                    ...
                    Err(err) => {
                        ...
                        self.toasts.error("Failed to fetch update information").duration(Some(web_time::Duration::from_secs(15)));
