        ...
        match self.game_handler.stage {
            ...
            GameStage::NoMoreQuestions => egui::Window::new("Question").open(&mut self.state.windows.game_question.opened).show(ctx, |ui| {
                ...
                if self.game_handler.question_catalog.is_empty() {
                    ui.label("There are no more questions to be chosen from as this question pack is empty. You have to choose a different question pack from the game settings.");
                } else {
                ...
                ui.horizontal(|ui| {
                    ui.add_enabled_ui(!self.game_handler.question_catalog.is_empty(), |ui| {
