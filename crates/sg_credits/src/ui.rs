pub fn render_credits_window(opened: &mut bool, ctx: &egui::Context) -> Option<egui::InnerResponse<Option<()>>> {
    egui::Window::new("Credits").open(opened).show(ctx, |ui| {
        render_credits_inner(ui);
    })
}

pub fn render_credits_inner(ui: &mut egui::Ui) {
